...
- 访问路径：`/admin.html`（需已登录且具备管理员权限）。
- 管理员判定：
  - 环境变量 `ADMIN_USERNAMES`（可选，逗号分隔用户名）；或
  - `ADMIN_MIN_TRUST_LEVEL`（默认 4，达到该信任等级即视为管理员）。
  - 配置文件：`backend/.env`
- 能力：
  - 查看最近活跃会话列表与会话详情（含内部状态）。
  - 清空指定玩家会话。
  - 标记惩戒（轻度亵渎 / 重度渎道），将在后端逻辑中按规则处理。

示例 `.env` 片段：

```dotenv
ADMIN_USERNAMES="alice,bob"
ADMIN_MIN_TRUST_LEVEL=4
```

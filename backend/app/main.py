...
admin_router = APIRouter(prefix="/api/admin", dependencies=[Depends(auth.get_current_admin_user)])
...
@admin_router.get("/sessions/recent")
async def admin_recent_sessions(limit: int = 20):
    return state_manager.get_most_recent_sessions_admin(limit=limit)


@admin_router.get("/sessions/{player_id}")
async def admin_get_session(player_id: str):
...
@admin_router.post("/sessions/{player_id}/clear")
async def admin_clear_session(player_id: str):
    await state_manager.clear_session(player_id)
    return {"status": "ok"}


@admin_router.post("/punish")
async def admin_flag_punishment(req: PunishRequest):
    level = req.level.strip()
    if level not in ("轻度亵渎", "重度渎道"):
        raise HTTPException(status_code=400, detail="Invalid level; must be 轻度亵渎 or 重度渎道")
    await state_manager.flag_player_for_punishment(req.player_id, level=level, reason=req.reason or "admin")
    return {"status": "ok"}

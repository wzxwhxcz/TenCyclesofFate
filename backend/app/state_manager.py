...
def get_most_recent_sessions_admin(limit: int = 20) -> list[dict]:
    """
    Admin-only: returns recent sessions with extra fields for inspection.
    Does not include internal_history to keep payload light; use get_session for full.
    """
    valid_sessions = [s for s in SESSIONS.values() if "last_modified" in s]
    sorted_sessions = sorted(valid_sessions, key=lambda s: s["last_modified"], reverse=True)
    results = []
    for s in sorted_sessions[:limit]:
        results.append({
            ...
            "is_in_trial": s.get("is_in_trial"),
            "opportunities_remaining": s.get("opportunities_remaining"),
            "daily_success_achieved": s.get("daily_success_achieved"),
        })
    return results
